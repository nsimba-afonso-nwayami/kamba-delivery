...
export default function CompleteEntregador() {
  ...
  const uploadBtnStyle = "w-full h-32 border-2 border-dashed border-gray-200 rounded-xl flex flex-col items-center justify-center gap-2 text-gray-400 hover:border-red-700 hover:text-red-700 transition-all font-bold text-xs bg-gray-50/30 group";
    ...
    <>
      ...
      <section className="min-h-screen py-24 bg-gray-50 px-6 flex items-center justify-center">
        <div className="w-full max-w-4xl bg-white rounded-2xl shadow-xl shadow-gray-200/50 p-10 md:p-14 border border-gray-100 relative">
          ...
          {/* LOGO (ÍCONE DE MOTO PARA ENTREGADOR) */}
          ...
          <form className="space-y-8" onSubmit={(e) => e.preventDefault()}>
            ...
            <div className="bg-gray-50/30 p-8 rounded-2xl border-2 border-dashed border-gray-200 text-center">
              ...
              <input type="file" ref={fileRef} accept="image/*" capture="user" className="hidden" onChange={(e) => handleFileChange(e, setPhotoPreview)} />

              {!photoPreview ? (
                <button type="button" onClick={() => fileRef.current.click()} className="mx-auto flex flex-col items-center gap-3 text-red-700 hover:text-red-900 transition-colors group">
                ...
                <div className="relative inline-block">
                  ...
                  <button type="button" onClick={() => setPhotoPreview(null)} className="absolute -top-2 -right-2 bg-red-700 text-white w-8 h-8 rounded-full flex items-center justify-center shadow-md hover:bg-red-900 transition-all"><i className="fas fa-times text-xs"></i></button>
                </div>
              )}
            </div>

            {/* GRID DE DOCUMENTOS (BI E CARTA) */}
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                {!biFrentePreview ? (
                  <button type="button" onClick={() => biFrenteRef.current.click()} className={uploadBtnStyle}>
                  ...
                  <div className="relative h-32 w-full">
                    ...
                    <button type="button" onClick={() => setBiFrentePreview(null)} className="absolute -top-2 -right-2 bg-red-700 text-white w-7 h-7 rounded-full flex items-center justify-center shadow-md"><i className="fas fa-times text-[10px]"></i></button>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                {!biVersoPreview ? (
                  <button type="button" onClick={() => biVersoRef.current.click()} className={uploadBtnStyle}>
                    <i className="fas fa-id-card text-xl group-hover:scale-110 transition-transform"></i>
                    Carregar Verso
                  </button>
                ) : (
                  <div className="relative h-32 w-full">
                    ...
                    <button type="button" onClick={() => setBiVersoPreview(null)} className="absolute -top-2 -right-2 bg-red-700 text-white w-7 h-7 rounded-full flex items-center justify-center shadow-md"><i className="fas fa-times text-[10px]"></i></button>
                  </div>
                )}
              </div>
              ...
              <div className="md:col-span-2 space-y-2">
                <label className="text-sm font-bold text-gray-700 ml-1">Carta de Condução</label>
                <input type="file" ref={cartaRef} accept="image/*" className="hidden" onChange={(e) => handleFileChange(e, setCartaPreview)} />
                {!cartaPreview ? (
                  <button type="button" onClick={() => cartaRef.current.click()} className={uploadBtnStyle}>
                    <i className="fas fa-id-card-clip text-xl group-hover:scale-110 transition-transform"></i>
                    Carregar Carta de Condução
                  </button>
                ) : (
                  <div className="relative h-40 w-full">
                    <img src={cartaPreview} alt="Carta" className="w-full h-full object-cover rounded-xl border border-gray-200 shadow-sm" />
                    <button type="button" onClick={() => setCartaPreview(null)} className="absolute -top-2 -right-2 bg-red-700 text-white w-8 h-8 rounded-full flex items-center justify-center shadow-md"><i className="fas fa-times text-xs"></i></button>
                  </div>
                )}
              </div>
            </div>
