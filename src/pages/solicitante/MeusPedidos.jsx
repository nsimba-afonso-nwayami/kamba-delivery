      ...
      <div className="max-w-5xl mx-auto pb-24">
        ...
        <div className="bg-white p-3 rounded-3xl border border-gray-100 shadow-sm flex flex-col md:flex-row gap-3 mb-8">
          ...
          <select
            ...
            className="px-6 py-3 bg-gray-50/50 rounded-2xl outline-none text-sm font-bold text-gray-600 border-none cursor-pointer hover:bg-gray-100 transition-colors"
          >
              ...
              <div key={pedido.id} className="bg-white border border-gray-100 rounded-3xl p-6 transition-all hover:shadow-xl hover:shadow-gray-200/40">
                <div className="flex flex-wrap justify-between items-start gap-4">
                  <div className="flex gap-4">
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-xl ${statusConfig[pedido.status].style.split(' ')[0]} ${statusConfig[pedido.status].style.split(' ')[1]}`}>
                      <i className={`fas ${statusConfig[pedido.status].icon}`}></i>
                    </div>
                    <div>
                      <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{pedido.id} • {distancia.toFixed(1)} km</span>
                      <h3 className="font-bold text-gray-800 text-lg leading-tight mb-3">{pedido.titulo}</h3>
                      
                      <div className="space-y-2">
                        <div className="flex items-center gap-3 text-xs font-bold text-gray-600">
                          <div className="w-5 h-5 flex items-center justify-center bg-red-50 rounded-full">
                             <i className="fas fa-circle-dot text-red-500 text-[8px]"></i>
                          </div>
                          <span className="text-gray-400 uppercase text-[9px] w-12 font-bold">Origem:</span>
                          <span className="text-gray-700 truncate max-w-62.5">{pedido.origem}</span>
                        </div>
                        <div className="flex items-center gap-3 text-xs font-bold text-gray-600">
                          <div className="w-5 h-5 flex items-center justify-center bg-red-50 rounded-full">
                             <i className="fas fa-location-dot text-red-800 text-[10px]"></i>
                          </div>
                          <span className="text-gray-400 uppercase text-[9px] w-12 font-bold">Destino:</span>
                          <span className="text-gray-700 truncate max-w-62.5">{pedido.destino}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <span className="text-xl font-black text-gray-900 tracking-tight">{precoTotal}</span>
