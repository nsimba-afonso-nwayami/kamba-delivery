    ...
    <>
      ...
      >
        ...
        <div className="mb-10 pt-2">
          <Link to="/dashboard/solicitante" className="text-xl font-black tracking-tight flex items-center gap-2">
        ...
        <nav className="space-y-2 flex-1">
          ...
          <Link 
            ...
            className={`${linkStyle} ${isActive("/dashboard/solicitante/pedidos") ? activeStyle : normalStyle}`}
          >
            <i className={`fas fa-box ${isActive("/dashboard/solicitante/pedidos") ? "text-white" : "text-rose-400"}`}></i>
            Meus Pedidos
          </Link>

          <Link 
            to="/dashboard/solicitante/historico" 
            className={`${linkStyle} ${isActive("/dashboard/solicitante/historico") ? activeStyle : normalStyle}`}
          >
            <i className={`fas fa-clock-rotate-left ${isActive("/dashboard/solicitante/historico") ? "text-white" : "text-rose-400"}`}></i>
            Histórico
          </Link>

          <Link 
            to="/dashboard/solicitante/configuracoes" 
            className={`${linkStyle} ${isActive("/dashboard/solicitante/configuracoes") ? activeStyle : normalStyle}`}
          >
            <i className={`fas fa-user-gear ${isActive("/dashboard/solicitante/configuracoes") ? "text-white" : "text-rose-400"}`}></i>
            Perfil & Conta
          </Link>
        </nav>

        {/* LOGOUT (Sempre no fundo) */}
        <div className="pt-6 border-t border-red-800/50">
          <button className="flex items-center gap-3 w-full px-4 py-3 rounded-xl text-rose-200/60 hover:text-white hover:bg-red-600 transition-all font-bold text-sm group">
